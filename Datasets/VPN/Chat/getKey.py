...
for name in files:
    ...
    if flag: # payload -  -e _ws.col.Info
        p = "tshark -r ./" + name[:-1] + " -T fields -E separator=, -E header=y -e _ws.col.Info > " + name.split('.')[0] + ".csv"
    else:
        p = "tshark -r ./" + name[:-1] + " -T fields -E separator=, -e _ws.col.Info > " + name.split('.')[0] + ".csv"
    flag = False
    print(p)
    os.system(p)
